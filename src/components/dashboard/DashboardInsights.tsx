...
const CONFIGS: Record<InsightType, { icon: React.ElementType; color: string; bg: string; border: string }> = {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3">
      {insights.map((insight, i) => {
        const cfg = CONFIGS[insight.type];
        const Icon = cfg.icon;
        return (
          <div
            key={i}
            className={`bg-axiom-card border ${cfg.border} rounded-xl p-4 flex flex-col gap-2`}
          >
